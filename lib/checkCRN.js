...
module.exports = (crn, currentStatus, user, db, cb) => {
  if (crn.length > 10) {
    ...
    request(`https://banner.aus.edu/axp3b21h/owa/bwckschd.p_disp_detail_sched?term_in=${config.misc.termID}&crn_in=${crn}`, function(err, res, body) {
      if (err) {
        cb('Something went wrong with querying Banner. Contact me. (b00073615@aus.edu) [Code: 1]')
      } else {

        //Virtual DOM - Emulates a Javascript Browser. Works 10% of the time.
        ...
        if ($('.errortext').length) {
          ...
          if (title.length > 0) {
            ...
            cb('Something went wrong with querying Banner. Contact me. (b00073615@aus.edu) [Code: 2]')
          }
