    ...
    db.sequelize.query('SELECT DISTINCT subject FROM crnStatuses').then(function(data) {
      if (data[0].length > 0) {
        ...
        function startCrawl() {
          ...
          //Wait 10 seconds for RAM to clear
          sleep.sleep(10)
          ...
          if (index > (subjects.length - 1)) {
            ...
            db.crnStatus.findAll({
              ...
              //Lets fetch the page
              fetchStatus(config.misc.termID, sub.subject, crns, db)
                .then(function() {
                ...
                .catch(function(e){
                  console.log(chalk.red(`ERROR for ${subject}: ${e}`))
                  startCrawl()
                })
