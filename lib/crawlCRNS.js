...
function parseBody(body, subject, crns, db, cb) {
  ...
  crns.forEach(function(crn, i) {
    ...
    if (status.length > 0) {
      ...
      //console.log(body)
      console.log(`Faulty crawl info: ${status}, ${config.misc.termID}, ${crn.dataValues.crn}, $(\`a[href="/axp3b21h/owa/bwckschd.p_disp_detail_sched?term_in=${config.misc.termID}&crn_in=${crn.dataValues.crn}"]\`).parent().closest('tr').next().find('td[colspan="1"]').text()`)
