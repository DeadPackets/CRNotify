...
//Functions
function parseBody(results, subject, crns, db, cb) {
  ...
  results.forEach(function(obj, i) {
    ...
    if (crnsProcessed == results.length) {
      cb(false)
    }
        ...
        function startCrawl() {
          ...
          if (index > (subjects.length - 1)) {
            ...
            db.crnStatus.findAll({
              ...
              //Lets fetch the page
              io.emit(`crawlCRN_${config.misc.secret}`, config.misc.termID, sub.subject, crns, function(err, result){
                if (err) {
                  console.log(chalk.red(`ERROR for ${sub.subject}`))
                  startCrawl()
                } else {
                  parseBody(result, sub.subject, crns, db, function(err){
                    if (err) {
                      console.log(chalk.red(`ERROR for ${sub.subject}`))
                      startCrawl()
                    } else {
